...
| Tests | `tests/` | Pytest + unittest-style modules; **no** `sys.path` hacks — install package first. Requires **`iav`** (Interval_Homogeneity) for `cardinality` tests. |
...
- **Development:** `pip install "git+https://github.com/LuisMRaimundo/Interval_Homogeneity.git"` then `pip install -e ".[dev]"`, then `pytest`, optional `ruff check .`, `mypy`.
